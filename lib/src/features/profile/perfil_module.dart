...
import 'package:domain_receitinhas/features/profile/domain/repositories/profile_repository_imp.dart';
...
class ProfileModule extends RegisterModule {
  ...
  void inicialize() {
    //Repository
    di.registerFactory<ProfileRepository>(() => ProfileRepositoryImp());

    //Controller
    di.registerFactory<ProfileController>(() => ProfileController(di(), di()));
    di.registerFactory<EditProfileController>(
      () => EditProfileController(di(), di()),
    );
  }
}
